...
class Aplicacao_Acl_Setup
{
	...
	$this->_acl = new Zend_Acl();
        $this->_initialize($idConstrutor);
		
    }
	
    protected function _initialize($idInicio)
    {
        $this->_setupRoles();
        $this->_setupResources();
        $this->_setupPrivileges($idInicio);
        $this->_saveAcl();
    ...
    {	$perfis=$this->_listaPerfis();
		...
		{
		$this->_acl->addRole( new Zend_Acl_Role($valor['id']),$valorAnt );
	...
	protected function _allowAdmin()
    {
		...
		$this->_acl->allow( '1', 'auth', array('index', 'login','logout'))
